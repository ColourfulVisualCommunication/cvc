...
Current position: **Phase 5 shipped** — Home, About, Our Story, Process, Services + detail pages, Portfolio, Blog, and Contact are all live at colourfulvisualcommunication.com (Cloudflare Workers), with admin CRUD for portfolio/posts/testimonials/client logos, build-time prerendering with meta/OG/JSON-LD, a full typography system, lead capture, and quotes (admin builder, one signed link, open-tracking, accept/decline with no client account). Now money: accepting a quote auto-creates a deposit invoice, the client pays via M-Pesa STK Push from that same quote link, Daraja's server-verified callback — never the browser — is what marks it paid, a retried callback is a proven no-op (idempotent by design, not just by luck), and a PDF receipt goes out by email. Admin can also record manual payments (bank transfer/cheque/cash) and reconcile a stuck pending payment. Services remain public-read-only (edited via seed data, not an admin panel) — the ladder rarely changes and a fixed price list didn't justify the CRUD yet. **Phase 6 next.** See `docs/02-build-plan.md` for full detail.
