...
Current position: **Phase 4 shipped** — Home, About, Our Story, Process, Services + detail pages, Portfolio, Blog, and Contact are all live at colourfulvisualcommunication.com (Cloudflare Workers), with admin CRUD for portfolio/posts/testimonials/client logos, build-time prerendering with meta/OG/JSON-LD, a full typography system, lead capture (a "Start a project" form → `lead` record → admin notification + enquirer acknowledgement by email → admin Leads inbox), and now quotes: an admin quote builder with line items and a deposit percentage, sent as one signed link (`/quote/:token`, no login) with open-tracking (viewed/accepted/declined timestamps), automatic expiry, one reminder, and accept/decline handled entirely by the client without an account. Services remain public-read-only (edited via seed data, not an admin panel) — the ladder rarely changes and a fixed price list didn't justify the CRUD yet. **Phase 5 next.** See `docs/02-build-plan.md` for full detail.
